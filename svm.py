...
with fiona.open(temp_point_loc) as shp:
    for feature in shp:
        ...
        if (siteID <= 0.2):                      # IF PPI is smaller then 0.2 put into 1 class
            temp_df['id'] = 1
            ...
            temp_df['id'] = 3
        elif ((siteID > 0.6) and (siteID <= 0.8)):# IF PPI is greater then 0.4  and smaller then 0.6 put into 4 class
            temp_df['id'] = 4
        else:
            temp_df['id'] = 5   # IF PPI is greater then 0.8  5 class
...
#test = metrics.f1_score(Y_test, Y_pred, average='weighted', labels=np.unique(Y_pred))
...
#img_reshape = pred_full.reshape((height, width, 3))
img_reshape = pred_full.reshape( height, width, 3)
img_reshape = np.moveaxis(img_reshape,-1 , 0)
print(img_reshape.size)
#Export the results
